...
// for verbose mode, define DISPLAY.
 #define DISPLAY
...
public:
    ...
    Iter<T> begin()
    {
        ...
        return Iter<T>(addr, addr, addr + __size);
    }
    ...
    Iter<T> end()
    {
        ...
        return Iter<T>(addr + __size, addr, addr + __size);
    }
...
template <class T, int size> typename list<GCInfo<T> >::iterator GCPtr<T, size>::findPtrInfo(T *ptr)
{
    typename list<GCInfo<T> >::iterator p;
